...
from .models import BankAccount
from .serializers import BankAccountSerializer
from Loan.models import Loan  # Import the Loan model
from django.shortcuts import get_object_or_404
from django.db.models import F  # Import F for field comparisons
...
class BankAccountSuspendUnsuspendView(APIView):
    ...
    def put(self, request):
        ...
        # Check if the account is blocked; a blocked account cannot be suspended
        if bank_account.status == 'blocked':
            return Response({"error": "Your account is blocked; can't be suspended or unsuspended!"}, status=status.HTTP_400_BAD_REQUEST)

        if action == 'suspend':
            # Check if the user has any loans that are not fully repaid
            outstanding_loans = Loan.objects.filter(user=bank_account.user, amount_paid__lt=F('amount'))
            if outstanding_loans.exists():
                return Response({"error": "You cannot suspend your account until all loans are fully repaid."}, status=status.HTTP_400_BAD_REQUEST)
